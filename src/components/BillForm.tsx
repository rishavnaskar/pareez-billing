...
export function BillForm() {
      ...
      const timeoutId = setTimeout(async () => {
        try {
          ...
          if (!createdCustomer) {
            ...
            createdCustomer = customers.find(
  ...
  useEffect(() => {
    ...
    const hasBlankService = services.some(
      (s) => !s.serviceName && !s.price && !s.discountAmount && !s.staffName,
    );
    const currentSubtotal = services.reduce(
      (sum, s) => sum + (s.price || 0),
      0,
    );
  ...
  const handleSaveBill = async () => {
    ...
    setLoading(true);
    try {
      const finalTotalAmount = subtotal - serviceDiscounts - discountAmount;
      ...
      const billData: Omit<Bill, "id" | "createdAt"> = {
        ...
        netPayableAmount: finalTotalAmount - walletUsage,
      ...
      if (savedBill) {
        ...
        // Create new bill with wallet processing
        const { billId, updatedWallet } = await processBillWithWallet(
          selectedCustomer.id,
          billData,
          walletUsage,
          cashback,
              ...
              {selectedCustomer && !savedBill && isEligible && resolvedRates && (
                <div className="mt-4 p-4 bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl border border-orange-200">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <TierBadge tier={selectedCustomer.wallet.tier} size="sm" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-white/70 rounded-lg p-2">
                      ...
                      <p className="font-bold text-orange-600">
                        {formatINR(selectedCustomer.wallet.balance)}
                      </p>
                    </div>
                    <div className="bg-white/70 rounded-lg p-2">
                      <p className="text-xs text-gray-500">Max Redeemable</p>
                      <p className="font-bold text-blue-600">
                        {formatINR(maxRedemption)}
                      </p>
                      ...
                      {useWallet && (
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            onClick={() => setWalletAmountToUse(maxRedemption)}
                            className="text-xs"
                          >
                            Use Max
                          </Button>
