...
import { ServiceItem, MembershipTier } from "./types";
import { TIER_CONFIG } from "./wallet";
...
export async function generateBillPDF(
  ...
  pdf.text("Unisex Professional Salon", pageWidth / 2, yPos, {
    align: "center",
  });
  yPos += 20;
  ...
  yPos += 6;
  pdf.text(
    `Phone: ${customerPhone ? maskPhoneNumber(customerPhone) : "N/A"}`,
    margin + 5,
    yPos,
  );
  yPos += 15;
  ...
  services.forEach((service, index) => {
    if (service.serviceName) {
      ...
      pdf.text(
        discount > 0 ? formatINRNoSymbol(-discount) : "-",
        margin + contentWidth * 0.6 + 4,
        yPos,
      );
      pdf.setTextColor(0, 0, 0);
      pdf.text(formatINRNoSymbol(total), pageWidth - margin - 4, yPos, {
        align: "right",
      });
      yPos += 8;
    }
  ...
  const calculatedSubtotal = services.reduce(
    (sum, service) => sum + (service.price || 0),
    0,
  );
  pdf.text(formatINRNoSymbol(calculatedSubtotal), pageWidth - margin, yPos, {
    align: "right",
  });
  yPos += 6;

  const serviceDiscounts = services.reduce(
    (sum, service) => sum + (service.discountAmount || 0),
    0,
  );
  if (serviceDiscounts > 0) {
    pdf.setTextColor(0, 128, 0);
    pdf.text("Service Discounts:", margin + 20, yPos);
    pdf.text(formatINRNoSymbol(-serviceDiscounts), pageWidth - margin, yPos, {
      align: "right",
    });
    yPos += 6;
  }
  ...
  if (discountAmount > 0) {
    ...
    pdf.text(formatINRNoSymbol(-discountAmount), pageWidth - margin, yPos, {
      align: "right",
    });
    yPos += 6;
  }
  ...
  if (options.walletAmountUsed && options.walletAmountUsed > 0) {
    ...
    pdf.text("Wallet Redemption:", margin + 20, yPos);
    pdf.text(
      formatINRNoSymbol(-options.walletAmountUsed),
      pageWidth - margin,
      yPos,
      { align: "right" },
    );
  ...
  pdf.text("BILL TOTAL", margin + 20, yPos);
  pdf.text(formatINRNoSymbol(totalAmount), pageWidth - margin, yPos, {
    align: "right",
  });
  yPos += 8;
  ...
  if (
    ...
    pdf.setTextColor(0, 128, 0);
    pdf.text("AMOUNT PAID", margin + 20, yPos);
    pdf.text(
      formatINRNoSymbol(options.netPayableAmount),
      pageWidth - margin,
      yPos,
      { align: "right" },
    );
  ...
  if (options.cashbackEarned && options.cashbackEarned > 0) {
    ...
    const tierName = options.customerTierAtPurchase
      ? TIER_CONFIG[options.customerTierAtPurchase].name
      : "";
    pdf.text(
      `YOUR REWARDS (${tierName} Member)`,
      margin + 5,
      yPos,
    );
    yPos += 6;
    ...
    pdf.setTextColor(0, 128, 0);
    pdf.text(
      `Cashback Earned: ${formatINRNoSymbol(options.cashbackEarned)}`,
      margin + 5,
      yPos,
    );

    if (options.walletBalanceAfter !== undefined) {
      pdf.setTextColor(255, 140, 0);
      pdf.text(
        `Wallet Balance: ${formatINRNoSymbol(options.walletBalanceAfter)}`,
        pageWidth - margin - 5,
        yPos,
        { align: "right" },
      );
  ...
  pdf.setFontSize(10);
  pdf.setTextColor(100, 100, 100);
  pdf.text("Thank you for choosing Pareez!", pageWidth / 2, yPos, {
    align: "center",
  });
  yPos += 5;
  pdf.text("We look forward to serving you again", pageWidth / 2, yPos, {
    align: "center",
  });
  yPos += 10;
  ...
  pdf.text("Follow us on social media:", pageWidth / 2, yPos, {
    align: "center",
  });
  yPos += 4;
